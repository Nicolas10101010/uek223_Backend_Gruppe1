...
public class UserProfileController {
    ...
    public ResponseEntity<UserProfileDTO> createProfile(
        ...
        try {
            UserProfileDTO createdProfile = userProfileService.createProfile(createDTO, currentUser);
    ...
    public ResponseEntity<UserProfileDTO> getOwnProfile(Authentication auth) {
        ...
        try {
            ...
            logger.info("UC2-API: Own profile retrieved successfully for user {} with profileId={}",
                    currentUser.getEmail(), profile.getId());

            return ResponseEntity.ok(profile);
        } catch (Exception e) {
            logger.error("UC2-API: Failed to retrieve own profile - User {} - Error: {}",
                    currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public ResponseEntity<UserProfileDTO> updateOwnProfile(
        ...
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC2-API: PUT /profiles/me - User {} updating own profile", currentUser.getEmail());

        try {
            UserProfileDTO updatedProfile = userProfileService.updateOwnProfile(updateDTO, currentUser);

            logger.info("UC2-API: Own profile updated successfully for user {} with profileId={}",
                    currentUser.getEmail(), updatedProfile.getId());

            return ResponseEntity.ok(updatedProfile);
        } catch (Exception e) {
            logger.error("UC2-API: Failed to update own profile - User {} - Error: {}",
                    currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public ResponseEntity<Void> deleteOwnProfile(Authentication auth) {
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC2-API: DELETE /profiles/me - User {} deleting own profile", currentUser.getEmail());

        try {
            userProfileService.deleteOwnProfile(currentUser);

            logger.info("UC2-API: Own profile deleted successfully for user {}", currentUser.getEmail());
            return ResponseEntity.noContent().build();
        } catch (Exception e) {
            logger.error("UC2-API: Failed to delete own profile - User {} - Error: {}",
                    currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public ResponseEntity<UserProfileDTO> getProfileById(
        ...
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC3-API: GET /profiles/{} - User {} retrieving profile", profileId, currentUser.getEmail());

        try {
            UserProfileDTO profile = userProfileService.getProfileById(profileId, currentUser);

            logger.info("UC3-API: Profile {} retrieved successfully by {}", profileId, currentUser.getEmail());
            return ResponseEntity.ok(profile);
        } catch (Exception e) {
            logger.error("UC3-API: Failed to retrieve profileId={} - User {} - Error: {}",
    ...
    public ResponseEntity<UserProfileDTO> updateProfile(
        ...
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC4-API: PUT /profiles/{} - User {} updating profile", profileId, currentUser.getEmail());

        try {
            UserProfileDTO updatedProfile = userProfileService.updateProfile(profileId, updateDTO, currentUser);

            logger.info("UC4-API: Profile {} updated successfully by {}", profileId, currentUser.getEmail());
            return ResponseEntity.ok(updatedProfile);
        } catch (Exception e) {
            logger.error("UC4-API: Failed to update profileId={} - User {} - Error: {}",
                    profileId, currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public ResponseEntity<Void> deleteProfile(
        ...
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC4-API: DELETE /profiles/{} - User {} deleting profile", profileId, currentUser.getEmail());

        try {
            userProfileService.deleteProfile(profileId, currentUser);

            logger.info("UC4-API: Profile {} deleted successfully by {}", profileId, currentUser.getEmail());
            return ResponseEntity.noContent().build();
        } catch (Exception e) {
            logger.error("UC4-API: Failed to delete profileId={} - User {} - Error: {}",
                    profileId, currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
    ...
    public ResponseEntity<Page<UserProfileDTO>> searchProfiles(
        ...
        User currentUser = ((UserDetailsImpl) auth.getPrincipal()).user();
        logger.info("UC5-API: GET /profiles - User {} searching profiles with filters", currentUser.getEmail());

        try {
            Page<UserProfileDTO> profiles = userProfileService.searchProfiles(
                    search, address, minAge, maxAge, pageable, currentUser);

            logger.info("UC5-API: Search completed by {} - Found {} profiles out of {} total",
                    currentUser.getEmail(), profiles.getNumberOfElements(), profiles.getTotalElements());

            return ResponseEntity.ok(profiles);
        } catch (Exception e) {
            logger.error("UC5-API: Search failed - User {} - Error: {}",
                    currentUser.getEmail(), e.getMessage(), e);
            throw e;
        }
    }
